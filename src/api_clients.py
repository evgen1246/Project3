...
class AeroplanesAPI:
    ...
    def get_aeroplanes(self, south: float, north: float, west: float, east: float) -> List[List[Any]]:
        """Получение самолетов в заданном регионе"""
        aircraft_data = self.opensky_api.get_data(lamin=south, lamax=north, lomin=west, lomax=east)
